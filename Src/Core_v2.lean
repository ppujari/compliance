...
namespace Src.Core_v2
open Lean

set_option linter.unusedVariables false

/-- Nested type for director details used in v2 rules. -/
structure Director where
  other_debarred_companies : List Nat
  deriving Repr, ToJson, FromJson

/-- Issuer data model for the mistral_7b_v1 ruleset. -/
structure Issuer where
  -- Disqualification and misconduct flags
  is_debarred : Bool
  has_debarred_directors : Bool
  selling_shareholders_debarred : Bool
  is_fraudulent : Bool
  is_fugitive : Bool
  directors : List Director

  -- Pre-IPO outstanding instruments and exemptions
  has_outstanding_convertibles : Bool
  has_esop_exemption : Bool
  has_sar_exemption : Bool
  has_mandatory_convertibles : Bool

  -- Financial history and thresholds
  ...
  -- Share status and OFS eligibility
  shares_fully_paid : Bool
  shares_held_duration_months : Nat
  has_held_equity_shares_for_1_year : Bool
  is_govt_entity : Bool
  ...
  -- Warrants and pricing
  warrant_tenure : Nat
  upfront_exercise_price : Bool

  -- Promoter contribution
  promoter_contribution : Nat

  -- Lock-in
  lock_in_period_minimum_contribution : Nat
  lock_in_period_excess_contribution : Nat
  lock_in_period : Nat
  lock_in_holders : String

  -- Intermediaries
  lead_manager_appointment : Bool
  deriving Repr, ToJson

/-- Input-friendly variant with optional fields for JSON decoding. -/
structure IssuerInput where
  is_debarred : Option Bool := none
  has_debarred_directors : Option Bool := none
  selling_shareholders_debarred : Option Bool := none
  is_fraudulent : Option Bool := none
  is_fugitive : Option Bool := none
  directors : Option (List Director) := none
  ...
  has_held_equity_shares_for_1_year : Option Bool := none
  ...
  upfront_exercise_price : Option Bool := none

  promoter_contribution : Option Nat := none

  lock_in_period_minimum_contribution : Option Nat := none
  lock_in_period_excess_contribution : Option Nat := none
  lock_in_period : Option Nat := none
  lock_in_holders : Option String := none

  lead_manager_appointment : Option Bool := none
  deriving Repr, FromJson

/-- Normalize an IssuerInput by filling defaults for missing fields. -/
def IssuerInput.normalize (x : IssuerInput) : Issuer :=
  ...
  , selling_shareholders_debarred := x.selling_shareholders_debarred.getD false
  , is_fraudulent := x.is_fraudulent.getD false
  , is_fugitive := x.is_fugitive.getD false
  , directors := x.directors.getD []
  ...
  , has_held_equity_shares_for_1_year := x.has_held_equity_shares_for_1_year.getD false
  ...
  , upfront_exercise_price := x.upfront_exercise_price.getD false

  , promoter_contribution := x.promoter_contribution.getD 0

  , lock_in_period_minimum_contribution := x.lock_in_period_minimum_contribution.getD 0
  , lock_in_period_excess_contribution := x.lock_in_period_excess_contribution.getD 0
  , lock_in_period := x.lock_in_period.getD 0
  , lock_in_holders := x.lock_in_holders.getD ""
